 ...
 <script>
     ...
     $(function() {
         ...
         $("#kode").load("page/proses_penjualan/pk.php", "op=ambilbarang");

         //meload isi tabel
         $("#barang").load("page/proses_penjualan/pk.php", "op=barang");
         ...
         $("#kode").change(function() {
             ...
             $.ajax({
                 url: "page/proses_penjualan/proses.php",
         ...
         $("#tambah").click(function() {
             ...
             $.ajax({
                 url: "page/proses_penjualan/pk.php",
    ...
    ?>
     ...
     <?php
        ...
        ?>
         ...
         <?php
            $nota = $_GET['nota'];
            $query = mysqli_query($koneksi, "SELECT sparepart.`jenis_sparepart`, jenis_sparepart.`harga`,detailpenjualan.`jumlah`,detailpenjualan.`subtotal` FROM detailpenjualan,jenis_sparepart,penjualan,sparepart WHERE detailpenjualan.`nonota`=penjualan.`nonota` AND detailpenjualan.`kode`=jenis_sparepart.`id_sparepart` AND sparepart.`kd_sparepart`=jenis_sparepart.`kd_sparepart`AND detailpenjualan.nonota='$nota'");
         ...
         <table class='table table-hover'>
             ...
             <?php
                ...
                ?>
                 <tr>
                     <td><?php echo $no++ ?></td>
                     <td><?php echo $r['jenis_sparepart'] ?></td>
                     <td><?php echo 'Rp. ', number_format($r['harga'], 0, ',', '.') ?></td>
                     <td><?php echo $r['jumlah'] ?></td>
                     <td><?php echo 'Rp. ', number_format($r['subtotal'], 0, ',', '.') ?></td>
                 </tr>
             <?php } ?>
             <tr>
                 <td colspan='4'>
                 ...
                 <td colspan='5'>
                     <h4><?php echo 'Rp. ', number_format($nomor['total'], 0, ',', '.') ?></h4>
                 </td>
             </tr>
         </table>
         <a href='?page=penjualan'><button type="button" class="btn btn-warning"><i class="fa fa-undo alt "></i> Kembali</button></a>
